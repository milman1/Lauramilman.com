...
The plan is pending and explicitly not executed. Seller Hub evidence reported 182 rows, with 51 old-end matches, 50 selected, zero extra, and zero missing after the R3006 exclusion. No eBay end was submitted and no Marketplace Connect disable writes occurred. The original 640 scope also contains 67 older keepers absent from the baseline keeper set; that historical discrepancy is not caused by this run. Issue #92 remains open.

The resumed-browser guard is mandatory: after any browser cleanup or turn boundary, freshly read the active SKU/item-ID set and Shopify status, compare against the 50-row plan, confirm R3006 remains excluded, and only then consider any review or action. A timeout and Mac lock interrupted the prior UI attempt.
...
The exact SKU search found no supplier result. The merchant requested source evidence; no product or price action is authorized.
...
Strategy work is merged. Keep the issue open pending the merchant’s goal approval.
...
The last successful direct watch source read at 2026-09-11T18:36:19Z returned 596 source rows and 51 gated rows, with 18 eligible; it performed zero writes. The next PR #116 run `34635412185` at 2026-09-11T18:49:24Z returned zero feed rows and zero writes, consistent with the source/API rate or availability failure. Earliest next condition dry-run is 19:05 UTC if no other calls are made and the Mac is available. No live condition repair is claimed.

### #114 — watch fact copy and eBay SEO

Code is complete and merged via PR #115. Keep live rollout open until a successful source dry-run creates an eligible plan, the eligible sync completes, and Shopify/eBay output is independently verified.

## Required next actions

1. Sol reviews this handoff and the exact 50-row #92 pending plan; no PR until review passes.
2. Restore a usable browser session and fresh-read the 182 active Seller Hub rows plus Shopify statuses before any #92 action. Preserve R3006 and compare every selected row.
...
5. Obtain the merchant’s requested login/source evidence for #93/#94 and complete Bucherer evidence for #95.
