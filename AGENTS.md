...
| Uploadify | Shopify app | Jewelry marketplace feed. Needs ACTIVE, SKU, qty > 0, Category. `uploadify_product.uploadify_active` = true is set only on qualifying Belgium Dia watches (`w-` handle, price, SKU, title, description, qty > 0). The Belgium sync deletes that metafield from every other Shopify product. Loose diamonds stay at qty 0, and their Uploadify metafields are removed, so it skips them. |
