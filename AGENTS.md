...
| Uploadify | Shopify app | Jewelry marketplace feed. Needs ACTIVE, SKU, qty > 0, Category. `uploadify_product.uploadify_active` = true is set only on qualifying Belgium Watch (ROMAN) pieces and TLV Watches (`TLV WATCHES LLC`) that pass the same papers and condition gates and whose source condition maps to Pre-Owned or Unworn. Retail Ready and Ultra Mint are pre-owned grades (`3000`), not new. Unrecognized conditions such as SLIDER are not flagged. `1000` is only source-confirmed Unworn with box and papers. Those TLV watches are imported by the Belgium sync, priced with the watch cost tiers, and published on the watch sales channels. They do not get the `ebay` tag. Qualifying watches also get `uploadify_product.vendor_sku` set to the same stock number as the variant SKU, which is the identifier Uploadify sends as the eBay Custom Label. Active lab-grown jewelry (Peaceful Diamonds and finished pieces tagged `lab-grown`, never loose stones) and gold chains in the chains collection keep the same listing switch when they are ACTIVE and have a title, description, price, a SKU on every variant, tracked quantity above zero, and a Shopify category. A single-variant piece also gets `vendor_sku` set to that SKU. Drafts and pieces missing any of those fields are not flagged. The sync deletes the listing switch, and that Vendor SKU with it, from every other Shopify product, including fine jewelry that is not in that set, estate pieces, and Vivid. Loose diamonds stay at qty 0, and their Uploadify metafields are removed, so it skips them. |
...
- Entry point for an Astra session: `docs/handoffs/ASTRA-START-HERE.md`.
