...
| Uploadify | Shopify app | Jewelry marketplace feed. Needs ACTIVE, SKU, qty > 0, Category. `uploadify_product.uploadify_active` = true is set only on qualifying Belgium Watch (ROMAN) pieces and current TLV Watches (`TLV WATCHES LLC`). The Belgium sync deletes that metafield from every other Shopify product. TLV stays archived on the storefront; the sync sets its quantity to 1 for Uploadify. Loose diamonds stay at qty 0, and their Uploadify metafields are removed, so it skips them. |
