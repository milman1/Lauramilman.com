...
| Uploadify | Shopify app | Jewelry marketplace feed. Needs ACTIVE, SKU, qty > 0, Category. Feed watches also need `uploadify_product.uploadify_active` = true, which the Belgium sync sets when the watch has a price, SKU, title, description, and qty > 0. Loose diamonds are deliberately kept at qty 0, and their Uploadify metafields are removed, so it skips them. |
