...
describe('Royal Chain listing builder', () => {
  ...
  it('builds structured copy, deduped media, and eBay item specifics from source facts', () => {
    ...
    expect(product.descriptionHtml).toBe(
      '<p>This 4mm cuban chain necklace is crafted in 14K Yellow Gold. Polished finish, lobster clasp, semi-solid construction. Available in 18 in and 20 in.</p>',
    );
    expect(product.descriptionHtml).not.toContain('<h2>');
    ...
    expect(product.metafields).toContainEqual(expect.objectContaining({ key: 'link', value: 'Cuban' }));
