...
2. **Normalize + gate** (`src/normalize.ts`): L colour / SI2 clarity floors for
   ...
   Comment, and belong to **Belgium Watch (ROMAN)** or **TLV Watches**. Vivid
   ...
   sync fetches a live stock allowlist for those two branches before normalizing.
   If that lookup fails or returns no stocks, the watch segment is protected and
   receives no writes or archive decisions; stock-prefix inference is never used.
   Dial-aftermarket notes in Comment still sell (`8114`). Brands outside the
   curated list still import when their stock belongs to an allowed branch and are tagged
...
matches with the live Belgium Watch and TLV allowlist. It never uses title, price, or mutable
